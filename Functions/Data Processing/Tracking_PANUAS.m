...
%         % Calculate Mahanalobis distance
        dist_check = MahanalobisDistance( ...
            ...
            radarsetup.tracking.RDCoupling);
        ...
        if (dist_check < dist) && (dist_check < radarsetup.tracking.dist_thresh)
            ...
            multi.track_list{tr}.meas.az = deg2rad(multi.detect_list{frame}.az(de));
            multi.track_list{tr}.meas.el = deg2rad(multi.detect_list{frame}.el(de));
...
for de = ind
    ...
    if (abs(multi.detect_list{frame}.vel(de)) > radarsetup.tracking.min_vel)
        ...
        meas = struct( ...
            ...
            'az',         deg2rad(multi.detect_list{frame}.az(de)), ...
            'el',         deg2rad(multi.detect_list{frame}.el(de)), ...
            'vel',        multi.detect_list{frame}.vel(de), ...
            'cart',       multi.detect_list{frame}.cart(:,de));
        
        % Construct kinematic uncertainty
        kin_pre = [meas.cart(1); meas.vel * cos(meas.az) * cos(meas.el); ...
            meas.cart(2); meas.vel * sin(meas.az) * cos(meas.el); ...
            meas.cart(3); meas.vel * sin(meas.el)];
        kin_pre = kin_pre(:);
...
for tr = hit_list
    ...
    [kin_est, kin_pre, unc_pre] = KalmanFilter_Step( ...
        ...
        radarsetup.tracking.EKF, radarsetup.tracking.RDCoupling);
